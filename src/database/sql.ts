import type { AgencyId, DownloadFilters } from '../api/consts.js';
...
function buildAvailableValuesSql(
  identifiers: DatabaseIdentifiers,
  column: 'route_id' | 'trip_id',
  agencyId?: AgencyId,
): string {
  const table = quoteIdentifierPath(identifiers.apiTable);
...
export function buildAvailableRoutesSql(
  identifiers: DatabaseIdentifiers,
  agencyId?: AgencyId,
): string {
...
export function buildAvailableTripsSql(
  identifiers: DatabaseIdentifiers,
  agencyId?: AgencyId,
): string {
